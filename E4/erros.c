...
void report_error_undeclared(ValorLexico *token) {
    char msg_erro[256];
    sprintf(msg_erro, "Identificador '%s' não declarado", token->valor_token);
...
void report_error_declared(ValorLexico *novo_token, Simbolo *simbolo_existente) {
    char msg_erro[256];
    sprintf(msg_erro, "Identificador '%s' já declarado (na linha %d)", 
...
void report_error_variable_as_function(ValorLexico *token) {
    char msg_erro[256];
    sprintf(msg_erro, "Identificador '%s' dito variável usado como uma função", token->valor_token);
    yyerror_base(msg_erro, token->num_linha);
    exit(ERR_VARIABLE);
}
...
void report_error_function_as_variable(ValorLexico *token) {
    char msg_erro[256];
    sprintf(msg_erro, "Identificador '%s' dito função usado como uma variável", token->valor_token);
    yyerror_base(msg_erro, token->num_linha);
    exit(ERR_FUNCTION);
}
...
void report_error_wrong_type_binary_op(int num_linha, TipoDados tipo1, TipoDados tipo2, const char* op) {
     char msg_erro[256];
     sprintf(msg_erro, "TIPOS NÃO COMPATÍVEIS: expressão '%s' %s '%s'",
...
void report_error_wrong_type_assignment(ValorLexico *id_token, TipoDados id_tipo, TipoDados expr_tipo) {
    char msg_erro[256];
    sprintf(msg_erro, "TIPOS NÃO COMPATÍVEIS: Atribuindo expressão do tipo '%s' ao identificador '%s' de tipo '%s'",
...
void report_error_wrong_type_initialization(ValorLexico *id_token, TipoDados var_tipo, TipoDados atrib_tipo) {
    char msg_erro[256];
    sprintf(msg_erro, "TIPOS NÃO COMPATÍVEIS: Variável '%s' de tipo '%s' inicializada com valor de tipo '%s'",
            id_token->valor_token, 
...
void report_error_wrong_type_return_expr(int num_linha, TipoDados expr_tipo, TipoDados decl_tipo) {
    char msg_erro[256];
    sprintf(msg_erro, "TIPOS NÃO COMPATÍVEIS: Tipo do valor de retorno ('%s') é diferente do tipo especificado ('%s')",
            tipo_para_string(expr_tipo),
...
void report_error_wrong_type_return_func(int num_linha, TipoDados decl_tipo, TipoDados func_tipo) {
    char msg_erro[256];
    sprintf(msg_erro, "TIPOS NÃO COMPATÍVEIS: Tipo do retorno ('%s') não é compatível com o tipo de retorno da função ('%s')",
            tipo_para_string(decl_tipo),
...
void report_error_wrong_type_if_else(int num_linha, TipoDados if_tipo, TipoDados else_tipo) {
    char msg_erro[256];
    sprintf(msg_erro, "TIPOS NÃO COMPATÍVEIS: Bloco 'if' com tipo '%s' e bloco 'else' com tipo '%s'",
