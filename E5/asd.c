...
asd_tree_t *asd_new(const char *label, TipoDados data_type, int num_linha, char *temp, ListaILOC *codigo)
{
  ...
  if (ret != NULL){
    ...
    if(temp != NULL){
      ret->temp = strdup(temp);
    }else{
      ret->temp = NULL;
    }

    ret->codigo = codigo; // Recebe endereço do pedaço de código (ou NULL)
...
void asd_free(asd_tree_t *tree)
{
  if (tree != NULL){
    ...
    free(tree->label);
    free(tree->temp);
    if(tree->codigo != NULL)liberar_lista_iloc(tree->codigo); // chama função para liberação da lista
    free(tree);
  }else{
...
void asd_add_child(asd_tree_t *tree, asd_tree_t *child, OrdemConcatenacao ordem_inv)
{
  if (tree != NULL && child != NULL){
    ...
    // Código adicionado para concatenação de código pai com filho/filhos
    if (child->codigo != NULL) {
        if (tree->codigo == NULL) {
            tree->codigo = child->codigo;
        } else {
            concatenar_listas(tree->codigo, child->codigo, ordem_inv);
        }
        child->codigo = NULL;
    }
